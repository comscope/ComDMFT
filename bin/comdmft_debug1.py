    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        ...
        if (ii>0):
            if (shell=='s'):
                ...
                l_char='3'
            print('u_table shape'+str(shape(u_table)), file=control['h_log'])
            print('slater_u shape'+str(shape(slater_u[str(ii)])), file=control['h_log'])                        
            u_table=column_stack((u_table, slater_u[str(ii)]))
            w_table=column_stack((w_table, slater_w[str(ii)]))
            v_table=column_stack((v_table, slater_v[str(ii)]))
        ...
        if (control['method']=='lda+dmft'):
            ...
            for jj in range(control['n_omega']):
                ...
                for ii in sorted(set(control['impurity_problem_equivalence'])):
                    ...
                    sig_dc_vec=imp_from_mat_to_array(sig_dc,imp[str(abs(ii))]['impurity_matrix'])
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                ...
                dc_vec=imp_from_mat_to_array(sig_hf,imp[str(abs(ii))]['impurity_matrix'])
...
def run_comwann(control,wan_hmat):
    ...
    run_string=control['mpi_prefix_wannier']+' '+control['comsuitedir']+"/ComWann_debug1"
    cmd = run_string
    print(cmd, file=control['h_log'])
