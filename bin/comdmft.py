...
def read_comdmft_ini():
	...
	print >> control['h_log'], 'impurity_matrix overwritten to be independent'
	...
	temp=map(int,np.loadtxt(control['initial_lattice_dir']+'/k_tau_freq.dat'))
	...
	# print >> control['h_log'], 'mpi_prefix_coulomb', control['mpi_prefix_coulomb']
	...
	if (control['method']=='lqsgw+dmft'):		
	    print >> control['h_log'], 'do_wannier', control['do_wannier']
	    print >> control['h_log'], 'do_coulomb', control['do_coulomb']
	    print >> control['h_log'], 'do_dc', control['do_dc']
...
#   in wan_hmat
	...
	if (str(-int(key)) in control['impurity_problem_equivalence']):
	    imp[key]['para']=False
	else:
	    imp[key]['para']=True
	...
	if (control['additional_unitary_transform_impurity']==0):
	    ...
	    print >> control['h_log'], "impurity_matrix reset"
	...
	print >> control['h_log'], 'here'
	print >> control['h_log'], type(imp[key]['impurity_matrix'])
	
	print >> control['h_log'], imp[key]['impurity_matrix']
	print >> control['h_log'], 'here'
    ...
    # print
    print >> control['h_log'], 'top_dir', control['top_dir']
    if (control['method']=='lda+dmft'):
	print >> control['h_log'], 'lattice_directory', control['lattice_directory']
    elif (control['method']=='lqsgw+dmft'):    
	print >> control['h_log'], 'coulomb_directory', control['coulomb_directory']    
    print >> control['h_log'], 'wannier_directory', control['wannier_directory'] 
    print >> control['h_log'], 'dc_directory', control['dc_directory'] 
    print >> control['h_log'], 'impurity_directory', control['impurity_directory'] 
    print >> control['h_log'], 'lowh_directory', control['lowh_directory']
...
def find_impurity_wan(control, wan_hmat):
	...
	if (control['spin_orbit']):
		...
		if (control['impurity_wan'][ip].count(0) !=0):
		    print >> control['h_log'], 'something wrong in find_impurity_wan'
		    sys.exit()		    		    
	else:
...
def initial_file_directory_setup(control):
    ...
    if (control['method'] == 'lda+dmft'):
	print >> control['h_log'], 'iter_num_impurity', control['iter_num_impurity'], '  max_iter_num_impurity', control['max_iter_num_impurity']
	print >> control['h_log'], 'iter_num_outer', control['iter_num_outer'], '  max_iter_num_outer', control['max_iter_num_outer']
    elif (control['method'] == 'lqsgw+dmft'):
	print >> control['h_log'], 'iter_num_impurity', control['iter_num_impurity']
	print >> control['h_log'], 'max_iter_num_impurity', control['max_iter_num_impurity']	
...
def find_place_to_restart(control):
	...
	# print >> control['h_log'], control['conv_table']
	...
	if (len(control['conv_table'])>0):
		...
		print >> control['h_log'], last_step, last_imp, last_imp_iter		
	    else:
		print >> control['h_log'], last_step, last_imp_iter
...
def initial_lattice_directory_setup(control):
    ...
    print >> control['h_log'], "initial dft directory setup done"
...
def read_wan_hmat_basis(control):
    ...
    print >> control['h_log'], basis_info
    print >> control['h_log'], 'reading wannier.inip to get basis information'
    return basis_info

def check_key_in_string(key,dictionary,control):
    if (key not in dictionary):
    	print >> control['h_log'], 'missing \''+key+'\' in '+dictionary['name']
    	sys.exit()
    return None
...
def overwrite_key_in_string(key,dictionary,dictionaryname,value,h_log):
    if (key in dictionary):
    	print >> control['h_log'], '\''+key+'\' in '+dictionaryname+' is overwritten'
    return value
...
def generate_initial_self_energy(control,imp):
    ...
    print >> control['h_log'], 'initial_self_energy generation done'
    os.chdir(control['top_dir'])
    return None
...
def delta_postprocessing(control,imp):
	...
	print >> control['h_log'], 'delta causality broken'
	sys.exit()    
...
def cal_dc_diagonal(control):
	...
	dc_vec=imp_from_mat_to_array(dc_mat_new,imp[str(abs(ii))]['impurity_matrix'])
    ...
    print >> control['h_log'], 'dc.dat generation done'
    os.chdir(control['top_dir'])
...
def cal_zinv_m1_diagonal(control):
    ...
    print >> control['h_log'], 'zinv_m1.dat generation done'
    os.chdir(control['top_dir'])
...
def prepare_impurity_solver(control,wan_hmat,imp):
	...
	if (control['spin_orbit']):
	    ...
	    equivalence_key=array(map(lambda row: map(lambda x: str(x) if x!='0' else '', map(str, row)), imp[key]['impurity_matrix']))
	else:
	    ndim=nimp_orb*2
	    e_imp_key=np.zeros((ndim, ndim))
	    trans_key=np.zeros((ndim, ndim))
	    ...
	    print >> control['h_log'], equivalence_key_int_mat
	    if (imp[key]['para']):
		mkey=key
		shiftval=0
	    else:
	    ...
	    print >> control['h_log'], mkey, shiftval
	    ...
	    equivalence_key[0:nimp_orb,0:nimp_orb]=map(lambda row: map(lambda x: str(x) if x!='0' else '', map(str, row)), equivalence_key_int_mat)
	    equivalence_key[nimp_orb:(2*nimp_orb),nimp_orb:(2*nimp_orb)]=map(lambda row: map(lambda x: str(x) if x!='0' else '', map(str, row)), equivalence_mkey_int_mat)
...
def cal_projected_mean_field_diagonal(control,imp):
	...
	h_vec=imp_from_mat_to_array(h_mat_new,imp[str(abs(ii))]['impurity_matrix'])
    ...
    print >> control['h_log'], 'projected_eig.dat generation done'
    os.chdir(control['top_dir'])
    return None
...
def cal_e_imp_diagonal(control):
    ...
    print >> control['h_log'], 'e_imp.dat generation done'
    os.chdir(control['top_dir'])
    return None
...
def cal_hyb_diagonal(control,imp):
    ...
    print >> control['h_log'], 'delta.dat generation done'
...
#     print >> control['h_log'], 'Delta.inp generation done'
...
def check_for_files(filepath, h_log):
	...
	print >> control['h_log'], 'missing:', filepath
	quit()
    return None
...
def solve_impurity_patrick(control):
    ...
    with open('./ctqmc.out', 'w') as logfile, open('./ctqmc.err', 'w') as errfile:
        ...
        if ret != 0:
            print >> control['h_log'], "Error in CTQMC. Check ctqmc.err for error message."
	    sys.exit()
    return None
...
def measure_impurity_patrick(control):
	...
	ret = subprocess.call(cmd,shell=True, stdout=logfile, stderr=errfile)
	if ret != 0:
	    print >> control['h_log'], "Error in EVALSIM. Check evalsim.err for error message."
	    sys.exit()
    print >> control['h_log'], "measure self-energy done"
    if (control['method']=='lqsgw+dmft'):
	iter_string='_'+str(control['iter_num_impurity'])
    elif (control['method']=='lda+dmft'):
	iter_string='_'+str(control['iter_num_outer'])+'_'+str(control['iter_num_impurity'])    
...
def write_json_all(control,imp,data_array,json_name):
    ...
    print >> control['h_log'], json_name+" written"	
    return None    
...
def impurity_postprocessing(control, imp, key):
    ...
    print >> control['h_log'], 'histogram information for impurity_'+imp['name']
    print >> control['h_log'], 'first moment', firstmoment
    print >> control['h_log'], 'second moment', secondmoment
    print >> control['h_log'], 'third moment', thirdmoment
...
def test_causality(filename):
	...
	print >> control['h_log'], "Causality in "+filename+" is broken"
    else:
	print >> control['h_log'], "Causality in "+filename+" is good"
    return causality
...
def write_transformation_matrix(control, filename):
	...
	if (control['additional_unitary_transform_impurity']==0):
	    ...
	    for jj in nimp_orb:
		tempmat[jj,:]=array(map(float,g.readline().split()))
...
def run_comlowh(control):
    ...
    with open(logfilename, 'w') as logfile, open(errfilename, 'w') as errfile:
        ret = subprocess.call(cmd, shell=True,stdout = logfile, stderr = errfile)
        if ret != 0:
            print >> control['h_log'], errormessage
    	    sys.exit()	    
    ...
    print >> control['h_log'], "comlowh done"
...
def comcoulomb_postprocessing(control,imp):
	...
	if (ii>0):
		...
		imp[str(ii)]['f2']=tempmat[-1,2]	    
		imp[str(ii)]['f4']=tempmat[-1,3]
		if (shell=='f'):
		    imp[str(ii)]['f6']=tempmat[-1,4]
    ...
    print >> control['h_log'], "comcoulomb done"
...
#     print >> control['h_log'], "Updates.json written"	    
#     return None
...
#     print >> control['h_log'], "Link.json written"	    
#     return None
...
def write_params_json(control,imp,e_imp_key,trans_key,equivalence_key,beta):
    ...
    print >> control['h_log'], "prepare_ctqmc:e_imp_done"

    with open('params.json','w') as outfile:
	json.dump(params_json,outfile, sort_keys=True, indent=4, separators=(',', ': '))
    print >> control['h_log'], "params.json written"	    
...
def write_dynamical_f0_json(imp):
    ...
    print >> control['h_log'], "DynF0.json written"	    
...
# 	if ret != 0:
# 	    print >> control['h_log'], "Error in atom. Check atom.out for error message."
# 	    sys.exit()	    

#     print >> control['h_log'], "prepare_ctqmc:atom done"
#     if (control['method']=='lqsgw+dmft'):
# 	iter_string='_'+str(control['iter_num_impurity'])
#     elif (control['method']=='lda+dmft'):
# 	iter_string='_'+str(control['iter_num_outer'])+'_'+str(control['iter_num_impurity'])    
...
def write_conv_wan(control):
	...
	temp2=temp1.split(')')[0]
	wan_info[:3,cnt]=map(float,temp2.split(','))
	cnt=cnt+1
    f.close()
...
def run_dft(control):
    ...
    with open(control['lattice_directory']+'/dft.out', 'w') as logfile, open(control['lattice_directory']+'/dft.err', 'w') as errfile:
        ret = subprocess.call(cmd, shell=True,stdout = logfile, stderr = errfile)
        if ret != 0:
            print >> control['h_log'], "Error in dft. Check dft.err for error message."
	    sys.exit()	    
...
#     else:
# 	print >> control['h_log'], 'couldn\'t find ', param
# 	quit()	
...
#     print >> control['h_log'], 'update, ef in dft', ef_old, ef_new
#     return None
...
def prepare_dft_input(control):
    ...
    print >> control['h_log'], "prepare_dft_input done"
...
def run_dc(control,imp):
	...
	if (control['method']=='lda+dmft'):
		...
		for ii in sorted(set(control['impurity_problem_equivalence'])):
		    ...
		    sig_dc_vec=imp_from_mat_to_array(sig_dc_new,imp[str(abs(ii))]['impurity_matrix'])
	    ...
	    for ii in sorted(set(control['impurity_problem_equivalence'])):
		sig_hf_new=dot(dot(trans_basis[str(ii)], sig_hf_dc[str(ii)]), conj(transpose(trans_basis[str(ii)])))
		dc_vec=imp_from_mat_to_array(sig_hf_new,imp[str(abs(ii))]['impurity_matrix'])
...
def generate_initial_transformation(control):
    os.chdir(control['lowh_directory'])
    print >> control['h_log'], control['impurity_wan']
...
def prepare_comlowh(control,wan_hmat,imp):
    ...
    print >> control['h_log'], "check and preparation done for the calculation of delta"
    os.chdir(control['top_dir'])
    return None
...
def run_comwann(control,wan_hmat):
    ...
    with open(control['wannier_directory']+'/comwann.out', 'w') as logfile, open(control['wannier_directory']+'/comwann.err', 'w') as errfile:
        ret = subprocess.call(cmd,shell=True, stdout = logfile, stderr = errfile)
        if ret != 0:
            print >> control['h_log'], "Error in comwann. Check comwann.err or OUT."
    	    sys.exit()
...
def lda_dmft(control,wan_hmat,imp):
	...
	print >> control['h_log'], "************************************************"
	print >> control['h_log'], "iteration:  "+str(control['iter_num_outer'])
	print >> control['h_log'], "************************************************"
	...
	print >> control['h_log'], "wannier function construction"
	if control['iter_num_outer']==1:
	...
	while control['iter_num_impurity'] <= control['max_iter_num_impurity']:
	    ...
	    print >> control['h_log'], '\n'
	    print >> control['h_log'], '*****   iter_num_impurity: ', str(control['iter_num_impurity']), '  *****'
...
def lqsgw_dmft(control,wan_hmat,imp):
    ...
    print >> control['h_log'], '*****   prepare dc  *****'    
