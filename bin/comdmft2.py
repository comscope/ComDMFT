...
def read_comdmft_ini():
    ...
    if (control['additional_unitary_transform_impurity'] != 0):
        ...
        print('impurity_matrix overwritten to be independent', file=control['h_log'],flush=True)
...
def prepare_impurity_solver(control,wan_hmat,imp):
    ...
    trans_basis=read_impurity_mat_static(control,control['top_directory']+'/trans_basis.dat')
...
def impurity_postprocessing(control, imp, key):
    ...
    histo[:,1]=histo_temp
    nn=json.load(open('params.obs.json'))["scalar"]["N"][0]
...
def write_params_json(control,imp,e_imp_key,trans_key,equivalence_key,beta):
    ...
    if (control['spin_orbit']):
        ...
        params_json["basis"]["type"]="product real"
    ...
    if ('dc_mat_to_read' in control):
        ...
        if (control['method']=='lda+dmft'):
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                ...
                dc_vec=imp_from_mat_to_array(sig_hf[str(ii)],imp[str(abs(ii))]['impurity_matrix'])
