...
def open_h_log(control):
    ...
    print('',                                 file=control['h_log'])
    print('*********************************',file=control['h_log'])
    print('             ComDMFT',             file=control['h_log'])
    print('*********************************',file=control['h_log'])
    print('',                                 file=control['h_log'])
    #DEBUG
    control['h_log'].flush()
...
def read_comdmft_ini():
    ...
    if not control['comsuitedir']:
        print("Error: Environment variable COMSUITE_BIN is not defined.", file=control['h_log'])
        sys.exit()
    ...
    control['sigma_mix_ratio']=control.get('sigma_mix_ratio', 0.1)
    # control['ef_mix_ratio']=control.get('ef_mix_ratio', 0.1)    
    ...
    if (control['additional_unitary_transform_impurity'] != 0):
        ...
        print('impurity_matrix overwritten to be independent', file=control['h_log'])
    ...
    if ('initial_self_energy' in control):
        ...
        if (control['additional_unitary_transform_impurity']!=0):
            check_key_in_string('initial_trans_basis', control,control)
    ...
    if ('mpi_prefix_coulomb' in control):
        ...
        # print('mpi_prefix_coulomb', control['mpi_prefix_coulomb'], file=control['h_log'])
    ...
    if (control['restart']):
        ...
        if (control['method']=='lqsgw+dmft'):
            print('do_wannier', control['do_wannier'], file=control['h_log'])
            print('do_coulomb', control['do_coulomb'], file=control['h_log'])
            print('do_dc',      control['do_dc'],      file=control['h_log'])
...
#   in wan_hmat
    ...
    for key, value in imp.items():
        ...
        if (control['additional_unitary_transform_impurity']==0):
            ...
            print("impurity_matrix reset", file=control['h_log'])
        ...
        print('here',                            file=control['h_log'])
        print(type(imp[key]['impurity_matrix']), file=control['h_log'])

        print(imp[key]['impurity_matrix'],       file=control['h_log'])
        print('here',                            file=control['h_log'])
        ...
        imp[key]['susceptibility_cutoff']=imp[key].get('susceptibility_cutoff', 300)
    ...
    # print
    print('top_dir', control['top_dir'],                         file=control['h_log'])
    if (control['method']=='lda+dmft'):
        print('lattice_directory', control['lattice_directory'], file=control['h_log'])
    elif (control['method']=='lqsgw+dmft'):    
        print('coulomb_directory', control['coulomb_directory'], file=control['h_log'])
    print('wannier_directory', control['wannier_directory'],     file=control['h_log'])
    print('dc_directory', control['dc_directory'],               file=control['h_log'])
    print('impurity_directory', control['impurity_directory'],   file=control['h_log'])
    print('lowh_directory', control['lowh_directory'],           file=control['h_log'])
        ...
        if (control['spin_orbit']):
            if (control['impurity_problem'][ip][1].lower()=='f'):
                ...
                if (control['impurity_wan'][ip].count(0) !=0):
                    print('something wrong in find_impurity_wan', file=control['h_log'])
                    sys.exit()
        else:
...
def initial_file_directory_setup(control):
    ...
    if (control['method'] == 'lda+dmft'):
        print('iter_num_impurity', control['iter_num_impurity'], '  max_iter_num_impurity', control['max_iter_num_impurity'], file=control['h_log'])
        print('iter_num_outer', control['iter_num_outer'], '  max_iter_num_outer', control['max_iter_num_outer'],             file=control['h_log'])
    elif (control['method'] == 'lqsgw+dmft'):
        print('iter_num_impurity', control['iter_num_impurity'],         file=control['h_log'])
        print('max_iter_num_impurity', control['max_iter_num_impurity'], file=control['h_log'])
...
def find_place_to_restart(control):
    ...
    if (control['method']=='lqsgw+dmft'):
        ...
        # print(control['conv_table'], file=control['h_log'])
        ...
        if (len(control['conv_table'])>0):
            ...
            if (len(control['conv_table'][-1][0].strip().split('_')) > 1):
                ...
                print(last_step, last_imp, last_imp_iter, file=control['h_log'])
            else:
                print(last_step, last_imp_iter, file=control['h_log'])
...
def initial_lattice_directory_setup(control):
    ...
    print("initial dft directory setup done", file=control['h_log'])
...
def read_wan_hmat_basis(control):
    ...
    print('reading wannier.inip to get basis information', file=control['h_log'])
    return basis_info

def check_key_in_string(key,dictionary,control):
    if (key not in dictionary):
        print('missing \''+key+'\' in '+dictionary['name'], file=control['h_log'])
        sys.exit()
    return None
...
def overwrite_key_in_string(key,dictionary,dictionaryname,value,h_log):
    if (key in dictionary):
        print('\''+key+'\' in '+dictionaryname+' is overwritten', file=control['h_log'])
    return value
...
def generate_initial_self_energy(control,imp):
    ...
    if ('initial_self_energy' in control):
        ...
        shutil.copy(control['top_dir']+'/sig_dc.dat', './sig.dat')
        shutil.copy(control['top_dir']+'/sig_dc_hf.dat', './sig_hf.dat')
        shutil.copy(control['top_dir']+'/sig_dc_hf.dat', control['top_dir']+'/sig_hf.dat')
        shutil.copy(control['top_dir']+'/sig_dc.dat', control['top_dir']+'/sig.dat')        
    ...
    labeling_file('./sig_hf.dat', iter_string)    
    print('initial_self_energy generation done', file=control['h_log'])
    os.chdir(control['top_dir'])
    return None
...
def delta_postprocessing(control,imp):
    ...
    if (delta_causality ==0):
        print('delta causality broken', file=control['h_log'])
        sys.exit()    
...
def cal_dc_diagonal(control):
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        # dc_mat_new=dot(dot(trans_basis[str(ii)], dc_mat[str(ii)]), conj(transpose(trans_basis[str(ii)])))
    ...
    print('dc.dat generation done', file=control['h_log'])
    os.chdir(control['top_dir'])
...
def cal_zinv_m1_diagonal(control):
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        # zinv_m1_mat_new=dot(dot(trans_basis[str(ii)], zinv_m1_mat[str(ii)]), conj(transpose(trans_basis[str(ii)])))
        # zinv_m1_vec=imp_from_mat_to_array(zinv_m1_mat_new,imp[str(abs(ii))]['impurity_matrix'])
    ...
    print('zinv_m1.dat generation done', file=control['h_log'])
    os.chdir(control['top_dir'])
...
def prepare_impurity_solver(control,wan_hmat,imp):
    ...
    for key, value in imp.items(): 
        ...
        if (control['spin_orbit']):
            ...
            e_imp_key=np.zeros((ndim, ndim))
            trans_key=np.zeros((ndim, ndim))
            equivalence_key=np.zeros((ndim,ndim),dtype='str')
            equivalence_key_int_mat=array(imp[key]['impurity_matrix'])
            print(equivalence_key_int_mat, file=control['h_log'])
            if (imp[key]['para']):
                mkey=key
                shiftval=0
            else:
            ...
            print(mkey, shiftval, file=control['h_log'])
            #
...
def run_impurity_solver(control,imp):
    ...
    for key, value in imp.items():
        ...
        green[key], sigma_bare[key], sigma[key], sigma_to_delta[key], sigma_hf_mixed[key],sigma_hf[key]=impurity_postprocessing(control, imp, key)
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        ...
        for kk in range(n_iio):
            if (ii<0):
                ...
                pp=kk        
            h.write(str(real(sigma_hf[str(abs(ii))][pp]))+'   '+str(imag(sigma_hf[str(abs(ii))][pp]))+'    ')
    h.close()

    h=open('./sig_hf_bare.dat', 'w')
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        ...
        for kk in range(n_iio):
            if (ii<0):
                ...
                pp=kk        
            h.write(str(real(sigma_hf_new[str(abs(ii))][pp]))+'   '+str(imag(sigma_hf_new[str(abs(ii))][pp]))+'    ')
    h.close()                    

    shutil.copy('./sig_hf.dat', control['top_dir'])                    
    ...
    labeling_file('./sig.dat',iter_string)
    labeling_file('./sig_hf.dat',iter_string)
    labeling_file('./sig_hf_bare.dat',iter_string)        
...
def cal_projected_mean_field_diagonal(control,imp):
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):    
        # h_mat_new=dot(dot(trans_basis[str(ii)], hmat[str(ii)]), conj(transpose(trans_basis[str(ii)])))
    ...
    print('projected_eig.dat generation done', file=control['h_log'])
    os.chdir(control['top_dir'])
    return None
...
def cal_e_imp_diagonal(control):
    ...
    dc=np.loadtxt(control['dc_directory']+'/dc.dat')
    hf=np.loadtxt(control['top_dir']+'/sig_hf.dat')    

    f=open('e_imp.dat', 'w')
    f.write("  ".join(map(str, eig-dc-hf))+'\n')
    f.close()
    ...
    print('e_imp.dat generation done', file=control['h_log'])
    os.chdir(control['top_dir'])
    return None
...
def cal_hyb_diagonal(control,imp):
    ...
    print('delta.dat generation done', file=control['h_log'])
...
#     print('Delta.inp generation done', file=control['h_log'])
...
def check_for_files(filepath, h_log):
    if len(glob.glob(filepath))==0:
        print('missing:', filepath, file=control['h_log'])
        quit()
    return None
...
def solve_impurity_patrick(control):
    ...
    with open('./ctqmc.out', 'w') as logfile, open('./ctqmc.err', 'w') as errfile:
        ...
        if ret != 0:
            print("Error in CTQMC. Check ctqmc.err for error message.", file=control['h_log'])
            sys.exit()
    return None
...
def measure_impurity_patrick(control):
    ...
    with open('./evalsim.out', 'w') as logfile, open('./evalsim.err', 'w') as errfile :
        ret = subprocess.call(cmd,shell=True, stdout=logfile, stderr=errfile)
        if ret != 0:
            print("Error in EVALSIM. Check evalsim.err for error message.", file=control['h_log'])
            sys.exit()
    print("measure self-energy done", file=control['h_log'])
    if (control['method']=='lqsgw+dmft'):
        iter_string='_'+str(control['iter_num_impurity'])
    elif (control['method']=='lda+dmft'):
        iter_string='_'+str(control['iter_num_outer'])+'_'+str(control['iter_num_impurity'])    
...
def write_json_all(control,imp,data_array,json_name):
    ...
    print(json_name+" written", file=control['h_log'])
    return None    
...
def read_dynamical_function_from_jsonfile(jsonfile, dict_name):
    Sig_temp=json.load(open(jsonfile))[dict_name]
    n_omega=len(Sig_temp['1']["function"]['real'])
    n_iio=len(Sig_temp.keys())
    dat1=zeros((n_omega, n_iio), dtype='complex')
    dat2=zeros((n_iio), dtype='complex')    
    for key, value in Sig_temp.items():    
        dat1[:,int(key)-1]=array(Sig_temp[key]["function"]['real'])+array(Sig_temp[key]["function"]['imag'])*1j-Sig_temp[key]["moments"][0]
        dat2[int(key)-1]=Sig_temp[key]["moments"][0]
    return dat1, dat2
...
def impurity_postprocessing(control, imp, key):
    ...
    labeling_file('./observables.json',iter_string)
    labeling_file('./params.meas.json',iter_string)    

    histo_temp=json.load(open('observables.json'))["expansion histogram"]

    histo=np.zeros((shape(histo_temp)[0], 2))
    histo[:,0]=arange(shape(histo_temp)[0])
    histo[:,1]=histo_temp
    nn=json.load(open('observables.json'))["scalar"]["N"][0]
    ctqmc_sign=json.load(open('observables.json'))["sign"]
    ...
    print('histogram information for impurity_'+imp['name'], file=control['h_log'])
    print('first moment',  firstmoment,                      file=control['h_log'])
    print('second moment', secondmoment,                     file=control['h_log'])
    print('third moment',  thirdmoment,                      file=control['h_log'])

    # previous_iter_string='_'.join(map(str,iter_string.split('_')[:-1]))+'_'+str(int(iter_string.split('_')[-1])-1)    

    green=read_function_from_jsonfile('./observables.json',"green")
    sigma_bare, sigma_hf_new=read_dynamical_function_from_jsonfile('./observables.json',"self-energy")

    sigma_hf_old=array_impurity_static(control,imp,control['impurity_directory']+'/sig_hf.dat')
    sigma_hf_mixed=np.zeros(np.shape(sigma_bare), dtype='complex')

    n_iio=amax(imp[key]['impurity_matrix'])
    
    if (imp[key]['para']):
        sigma_hf_mixed=(sigma_hf_old[key])*(1.0-control['sigma_mix_ratio'])+(sigma_hf_new)*control['sigma_mix_ratio']
    else:
        mkey=str(-int(key))
        sigma_hf_mixed[:,:n_iio]=(sigma_hf_old[key])*(1.0-control['sigma_mix_ratio'])+(sigma_hf_new[:,:n_iio])*control['sigma_mix_ratio']                
        sigma_hf_mixed[:,n_iio:]=(sigma_hf_old[mkey])*(1.0-control['sigma_mix_ratio'])+(sigma_hf_new[:,n_iio:])*control['sigma_mix_ratio']
    ...
    return green, sigma_bare, sigma, sigma_to_delta,sigma_hf_mixed,sigma_hf_new
...
def test_causality(filename):
    ...
    if ((dat[:,2::2]>0.0).any()):
        ...
        print("Causality in "+filename+" is broken", file=control['h_log'])
    else:
        print("Causality in "+filename+" is good",   file=control['h_log'])
    return causality
...
def run_comlowh(control):
    ...
    # ef_old=np.float(np.loadtxt('ef_mixed.dat'))
    ...
    with open(logfilename, 'w') as logfile, open(errfilename, 'w') as errfile:
        ret = subprocess.call(cmd, shell=True,stdout = logfile, stderr = errfile)
        if ret != 0:
            print(errormessage, file=control['h_log'])
            sys.exit()
    ...
    # ef_new=np.float(np.loadtxt('ef.dat'))
    # f=open('ef_mixed.dat','w')
    # f.write(str(ef_old*(1-control['ef_mix_ratio'])+ef_new*control['ef_mix_ratio'])+'\n')
    # f.close()
    ...
    # labeling_file('./ef_mixed.dat',iter_string)    

    os.chdir(control['top_dir'])


    print("comlowh done", file=control['h_log'])
...
def comcoulomb_postprocessing(control,imp):
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        ...
        if (ii>0):
            if (shell=='s'):
                ...
                l_char='3'    
            u_table=vstack((transpose(u_table), transpose(slater_u[str(ii)])))
            w_table=vstack((transpose(w_table), transpose(slater_w[str(ii)])))
            v_table.append(slater_v[str(ii)].tolist())
    ...
    with open(control['top_dir']+'/u_slater.dat', 'w') as outputfile:
        outputfile.write(tabulate(transpose(u_table), headers=slater_header, numalign="right",  floatfmt=".12f", tablefmt="plain"))
    with open(control['top_dir']+'/w_slater.dat', 'w') as outputfile:
        outputfile.write(tabulate(transpose(w_table), headers=slater_header, numalign="right",  floatfmt=".12f", tablefmt="plain"))
    ...
    print("comcoulomb done", file=control['h_log'])
...
#     print("Updates.json written"            , file=control['h_log'])
#     return None
...
#     print("Link.json written"            , file=control['h_log'])
#     return None
...
def write_params_json(control,imp,e_imp_key,trans_key,equivalence_key,beta):
    ...
    # basis
    params_json["basis"]={}
    params_json["basis"]["shell"]=imp['problem'].lower()
    if (control['spin_orbit']):
    ...
    if (params_json["basis"]["shell"]=='p') or (params_json["basis"]["shell"]=='d') or (params_json["basis"]["shell"]=='f') :
        params_json["hloc"]["two body"]["F2"]=imp['f2']
    if (params_json["basis"]["shell"]=='d') or (params_json["basis"]["shell"]=='f') :
        params_json["hloc"]["two body"]["F4"]=imp['f4']
    if (params_json["basis"]["shell"]=='f') :
    ...
    params_json["green cutoff"]=imp['green_cutoff']
    params_json["susceptibility cutoff"]=imp['susceptibility_cutoff']


    print("prepare_ctqmc:e_imp_done", file=control['h_log'])

    with open('params.json','w') as outfile:
        json.dump(params_json,outfile, sort_keys=True, indent=4, separators=(',', ': '))
    print("params.json written", file=control['h_log'])
...
def write_dynamical_f0_json(imp):
    ...
    print("DynF0.json written"    , file=control['h_log'])
...
#         if ret != 0:
#             print("Error in atom. Check atom.out for error message.", file=control['h_log'])
#             sys.exit()            

#     print("prepare_ctqmc:atom done", file=control['h_log'])
#     if (control['method']=='lqsgw+dmft'):
#         iter_string='_'+str(control['iter_num_impurity'])
#     elif (control['method']=='lda+dmft'):
#         iter_string='_'+str(control['iter_num_outer'])+'_'+str(control['iter_num_impurity'])    
...
def run_dft(control):
    ...
    with open(control['lattice_directory']+'/dft.out', 'w') as logfile, open(control['lattice_directory']+'/dft.err', 'w') as errfile:
        ret = subprocess.call(cmd, shell=True,stdout = logfile, stderr = errfile)
        if ret != 0:
            print("Error in dft. Check dft.err for error message.", file=control['h_log'])
            sys.exit()

    allfile=control['allfile']
    labeling_file('./'+allfile+'.out',iter_string)    
    shutil.move('./dft.out', './dft'+iter_string+'.out')
    print("dft calculation done", file=control['h_log'])
    os.chdir(control['top_dir'])
    return None
...
#     else:
#         print('couldn\'t find ', param, file=control['h_log'])
#         quit()        
...
#     print('update, ef in dft', ef_old, ef_new, file=control['h_log'])
#     return None
...
def prepare_dft_input(control):
    ...
    print("prepare_dft_input done", file=control['h_log'])
...
def write_conv_dc(control,imp):
    ...
    if (control['method']=='lqsgw+dmft'):
        for key, value in imp.items(): # for the ordered phase this part should be fixed
            ...
            os.chdir(control['dc_directory']+'/'+key)
            nimp_orb=len(imp[key]['impurity_matrix'])
            nn=np.trace(reshape(loadtxt('nimp.dat'), (nimp_orb, nimp_orb, 5), order='F')[:,:,3])
            # nimp_orb=len(imp[key]['impurity_matrix'])
            # nimp=np.trace(np.reshape(np.loadtxt('nimp.dat'), (nimp_orb,nimp_orb,5), order='F')[:,:,3])
            control['conv_table'].append(['dc_'+key,control['iter_num_impurity'],'good','','','','','',nn,'','',''])
            with open(control['top_dir']+'/convergence.log', 'w') as outputfile:
                outputfile.write(tabulate(control['conv_table'], headers=control['convergence_header'], numalign="right",  floatfmt=".5f"))
    return None
...
def run_dc(control,imp):
    if ('dc_mat_to_read' in control):
        ...
        if (control['method']=='lda+dmft'):
            ...
            if (control['iter_num_impurity']>0):

                sig_dc_old=generate_mat_from_array_impurity_dynamic(control,imp,control['dc_directory']+'/sig_dc.dat')

                # for ii in sorted(set(control['impurity_problem_equivalence'])):
                #     nimp_orb=len(imp[str(abs(ii))]['impurity_matrix'])                
                #     tempdat=reshape(loadtxt(control['dc_directory']+'/'+str(abs(ii))+'/sig_gwc_mat.dat')[:,1:], (control['n_omega'],2,nimp_orb,nimp_orb), order='F')
                #     sig_dc_old[str(ii)]=tempdat[:,0,:,:]+tempdat[:,1,:,:]*1j                

                sig_dc_hf_old=generate_mat_from_array_impurity_static(control,imp,control['dc_directory']+'/sig_dc_hf.dat')
                ...
                #     sig_dc_hf_old[str(ii)]=tempdat[:,:,0]+tempdat[:,:,1]*1j
            ...
            for key, value in imp.items(): # for the ordered phase this part should be fixed
                ...
                labeling_file('./sig_gw_mat.dat',iter_string)
                labeling_file('./sig_gwc_mat.dat',iter_string)                
                labeling_file('./exchange.dat',iter_string)
                labeling_file('./g_loc.dat',iter_string)
                labeling_file('./hartree.dat',iter_string)
                labeling_file('./nimp.dat',iter_string)

            os.chdir(control['dc_directory'])

            # labeling_file('./dc_mat.dat',iter_string)

            # if (control['iter_num_impurity']>1):
            #     sig_dc_old=loadtxt(control['dc_directory']+'/sig_dc_bare.dat')
            #     sig_dc_hf_old=loadtxt(control['dc_directory']+'/sig_dc_hf_bare.dat')                
            ...
            for jj in range(control['n_omega']):
                ...
                for ii in sorted(set(control['impurity_problem_equivalence'])):
                    tempmat=sig_dc[str(ii)][jj,:,:]
                    sig_dc_vec=imp_from_mat_to_array(tempmat, imp[str(abs(ii))]['impurity_matrix'])
                ...
                for ii in sorted(set(control['impurity_problem_equivalence'])):
                    if (control['iter_num_impurity']>0):
                        tempmat=sig_dc[str(ii)][jj,:,:]*control['sigma_mix_ratio']+sig_dc_old[str(ii)][jj,:,:]*(1-control['sigma_mix_ratio'])
                    else:
                        tempmat=sig_dc[str(ii)][jj,:,:]
                    sig_dc_vec=imp_from_mat_to_array(tempmat, imp[str(abs(ii))]['impurity_matrix'])
                    print(sig_dc_vec, np.reshape(np.stack((sig_dc_vec.real, sig_dc_vec.imag), 0), (len(sig_dc_vec)*2), order='F').tolist())
                    sig_omega=sig_omega+np.reshape(np.stack((sig_dc_vec.real, sig_dc_vec.imag), 0), (len(sig_dc_vec)*2), order='F').tolist()
                sig_table_mixed.append(sig_omega)
                


            with open(control['dc_directory']+'/sig_dc_bare.dat', 'w') as outputfile:
                outputfile.write(tabulate(sig_table, headers=control['sig_header'], floatfmt=".12f", numalign="right",  tablefmt="plain"))
            with open(control['dc_directory']+'/sig_dc.dat', 'w') as outputfile:
                outputfile.write(tabulate(sig_table_mixed, headers=control['sig_header'], floatfmt=".12f", numalign="right",  tablefmt="plain"))
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                nimp_orb=len(imp[str(abs(ii))]['impurity_matrix'])                                
                tempdat=reshape(loadtxt(control['dc_directory']+'/'+str(abs(ii))+'/hartree.dat')[:,3:], (nimp_orb,nimp_orb,2), order='F')+reshape(loadtxt(control['dc_directory']+'/'+str(abs(ii))+'/exchange.dat')[:,3:], (nimp_orb,nimp_orb,2), order='F')
                sig_dc_hf[str(ii)]=tempdat[:,:,0]+tempdat[:,:,1]*1j
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                # sig_hf_new=dot(dot(trans_basis[str(ii)], sig_dc_hf[str(ii)]), conj(transpose(trans_basis[str(ii)])))
                dc_vec=imp_from_mat_to_array(sig_dc_hf[str(ii)],imp[str(abs(ii))]['impurity_matrix'])
                sig_table.append(np.reshape(np.stack((dc_vec.real, dc_vec.imag), 0), (len(dc_vec)*2), order='F').tolist())

            with open(control['dc_directory']+'/sig_dc_hf_bare.dat', 'w') as outputfile:
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                ...
                if (control['iter_num_impurity']>0):
                    tempmat=sig_dc_hf[str(ii)]*control['sigma_mix_ratio']+sig_dc_hf_old[str(ii)]*(1-control['sigma_mix_ratio'])
                else:
                    tempmat=sig_dc_hf[str(ii)]
                dc_vec=imp_from_mat_to_array(tempmat,imp[str(abs(ii))]['impurity_matrix'])
                sig_table_mixed.append(np.reshape(np.stack((dc_vec.real, dc_vec.imag), 0), (len(dc_vec)*2), order='F').tolist())

            with open(control['dc_directory']+'/sig_dc_hf.dat', 'w') as outputfile:
                outputfile.write(tabulate(sig_table_mixed, headers=hf_header, floatfmt=".12f", numalign="right",  tablefmt="plain"))                
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                ...
                if (control['iter_num_impurity']>0):
                    tempmat=sig_dc[str(ii)][0,:,:]*control['sigma_mix_ratio']+sig_dc_old[str(ii)][0,:,:]*(1-control['sigma_mix_ratio'])
                else:
                    tempmat=sig_dc[str(ii)][0,:,:]
                dc=tempmat
                for jj in range(nimp_orb):
                    for kk in range(nimp_orb):
                        f.write(str(real(dc[jj,kk]))+'     0.0     ')
                        g.write(str(-imag(dc[jj,kk])/control['omega'][0])+'     0.0     ')
                    f.write('\n')
                    g.write('\n')
            f.close()
            g.close()                
            ...
            labeling_file('./dc_mat.dat',iter_string)
            labeling_file('./sig_dc_hf.dat',iter_string)
            labeling_file('./sig_dc_hf_bare.dat',iter_string)
            labeling_file('./sig_dc.dat',iter_string)
            labeling_file('./sig_dc_bare.dat',iter_string)                                    
            labeling_file('./zinv_m1_mat.dat',iter_string)
            shutil.copy('./sig_dc.dat', control['top_dir'])
            shutil.copy('./sig_dc_hf.dat', control['top_dir'])
            # if (control['iter_num_impurity']==0):
            #     shutil.copy('./sig_dc.dat', control['impurity_directory']+'/sig.dat')
            #     shutil.copy('./sig_dc_hf.dat', control['impurity_directory']+'/sig_hf.dat')                                
            os.chdir(control['top_dir'])
    return None

def generate_initial_transformation(control):
    os.chdir(control['lowh_directory'])
    print(control['impurity_wan'], file=control['h_log'])

    if ('initial_trans_basis' in control):
        shutil.copy(control['initial_trans_basis'], './trans_basis.dat')
    else:
        if control['additional_unitary_transform_impurity'] != 0:
            write_transformation_matrix(control,control['dc_directory']+'/local_spectral_matrix_ef.dat')
        else:
            f=open('trans_basis.dat', 'w')
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                prob_ind=control['impurity_problem_equivalence'].index(ii)
...
def prepare_comlowh(control,wan_hmat,imp):
    ...
    print("check and preparation done for the calculation of delta", file=control['h_log'])
    os.chdir(control['top_dir'])
    return None
...
def run_comwann(control,wan_hmat):
    ...
    with open(control['wannier_directory']+'/comwann.out', 'w') as logfile, open(control['wannier_directory']+'/comwann.err', 'w') as errfile:
        ret = subprocess.call(cmd,shell=True, stdout = logfile, stderr = errfile)
        if ret != 0:
            print("Error in comwann. Check comwann.err or OUT.", file=control['h_log'])
            sys.exit()
...
def lda_dmft(control,wan_hmat,imp):
    ...
    while control['iter_num_outer'] <= control['max_iter_num_outer']:
        ...
        print("************************************************", file=control['h_log'])
        print("iteration:  "+str(control['iter_num_outer']),      file=control['h_log'])
        print("************************************************", file=control['h_log'])
        ...
        print("wannier function construction", file=control['h_log'])
        if control['iter_num_outer']==1:
        ...
        while control['iter_num_impurity'] <= control['max_iter_num_impurity']:
            ...
            print("\n",                                                                        file=control['h_log'])
            print('*****   iter_num_impurity: ', str(control['iter_num_impurity']), '  *****', file=control['h_log'])
...
def lqsgw_dmft(control,wan_hmat,imp):
    ...
    print('*****   wannier  *****', file=control['h_log'])
    ...
    print('*****   Coulomb  *****', file=control['h_log'])
    ...
    print('*****   prepare dc  *****'    , file=control['h_log'])
