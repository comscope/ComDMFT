...
def prepare_impurity_solver(control,wan_hmat,imp):
    ...
    for key, value in imp.items(): 
        ...
        if (control['spin_orbit']):
            ...
            equivalence_key=list(map(lambda row: list(map(lambda x: str(x) if x!='0' else '', list(map(str, row)))), equivalence_key_int_mat))
            print(type(equivalence_key), file=control['h_log'])                        
        else:
            ndim=nimp_orb*2
            e_imp_key=np.zeros((ndim, ndim))
            trans_key=np.zeros((ndim, ndim))
            equivalence_key=np.zeros((ndim,ndim),dtype='str').tolist()
            equivalence_key_int_mat=array(imp[key]['impurity_matrix'])
            print(equivalence_key_int_mat, file=control['h_log'])
            if (imp[key]['para']):
                mkey=key
...
def run_impurity_solver(control,imp):
    ...
    for key, value in imp.items():
        ...
        green[key], sigma_bare[key], sigma[key], sigma_to_delta[key], hartree_vec[key]=impurity_postprocessing(control, imp, key)
    ...
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        ...
        hartree[ii]=zeros(n_iio, dtype='complex')
        for kk in range(n_iio):
            if (ii<0):
                pp=kk+n_iio
            else:
                pp=kk
                hartree[ii][kk]=hartree_vec[str(ii)][pp]

    sig_table=[]
    sig_header=control['sig_header'][1:]
    sig_header[0]='# '+sig_header[0]
    for ii in sorted(set(control['impurity_problem_equivalence'])):
        sig_table.append(np.reshape(np.stack((hartree[ii].real, hartree[ii].imag), 0), (len(hartree[ii])*2), order='F').tolist())

    with open('./sig_hartree.dat', 'w') as outputfile:
        outputfile.write(tabulate(sig_table, headers=sig_header, floatfmt=".12f", numalign="right",  tablefmt="plain"))	            
    ...
    labeling_file('./sig.dat',iter_string)
    labeling_file('./sig_hartree.dat',iter_string)

    shutil.copy('./sig_hartree.dat', control['top_dir'])        
...
def cal_e_imp_diagonal(control):
    ...
    dc=np.loadtxt(control['dc_directory']+'/dc.dat')
    hartree=np.loadtxt(control['top_dir']+'/sig_hartree.dat')        
...
def read_occ_from_jsonfile(jsonfile):
    dict_name="occupation"
    Sig_temp=json.load(open(jsonfile))[dict_name]
    n_iio=len(Sig_temp.keys())
    dat1=zeros(n_iio)
    for key, value in Sig_temp.items():    
        dat1[:,int(key)-1]=value
    return dat1
...
def impurity_postprocessing(control, imp, key):
    ...
    # hartree
    occupation=json.load(open('observables.json'))["occupation"]
    n_iio=amax(imp[key]['impurity_matrix'])    
    if (imp[key]['para']):
        tempvec=zeros(amax(imp[key]['impurity_matrix']), dtype=complex)
    else:
        tempvec=zeros(amax(imp[key]['impurity_matrix'])*2, dtype=complex)        
    nimp_orb=len(imp[key]['impurity_matrix'])    
    for keytemp, value in occupation.items():
        tempvec[int(keytemp)-1]=occupation[keytemp]
    if (imp[key]['para']):        
        n_mat=imp_from_array_to_mat(tempvec,imp[key]['impurity_matrix'])
        savetxt('nimp.dat', real(n_mat))        
    else:
        n_mat=zeros((nimp_orb*2, nimp_orb), dtype='complex')
        n_mat[:nimp_orb,:]=imp_from_array_to_mat(tempvec[:n_iio],imp[key]['impurity_matrix'])
        n_mat[nimp_orb:,:]=imp_from_array_to_mat(tempvec[n_iio:],imp[key]['impurity_matrix'])
        savetxt('nimp.dat', real(n_mat))                        
    ...
    if (imp[key]['para']):            
        trans=reshape(np.stack((trans_basis[key].real,trans_basis[key].imag),-1), (nimp_orb, 2*nimp_orb))
    else:
        mkey=str(-int(key))        
        trans=zeros((nimp_orb*2, nimp_orb*2))
        trans[:nimp_orb,:]=reshape(np.stack((trans_basis[key].real,trans_basis[key].imag),-1), (nimp_orb, 2*nimp_orb))
        trans[nimp_orb:,:]=reshape(np.stack((trans_basis[mkey].real,trans_basis[mkey].imag),-1), (nimp_orb, 2*nimp_orb))        
    
    savetxt('trans_dc.dat',trans)
    f=open('slater.dat', 'w')
    if (imp[key]['problem']=='s'):
        f.write(str(imp[key]['f0']+imp[key]['dynamical_f0'][0])+'\n')    
    elif (imp[key]['problem']=='p'):
        f.write(str(imp[key]['f0']+imp[key]['dynamical_f0'][0])+'  '+str(imp[key]['f2'])+'\n')    
    elif (imp[key]['problem']=='d'):
        f.write(str(imp[key]['f0']+imp[key]['dynamical_f0'][0])+'  '+str(imp[key]['f2'])+'  '+str(imp[key]['f4'])+'\n')
    elif (imp[key]['problem']=='f'):
        f.write(str(imp[key]['f0']+imp[key]['dynamical_f0'][0])+'  '+str(imp[key]['f2'])+'  '+str(imp[key]['f4'])+'  '+str(imp[key]['f6'])+'\n')
    f.close()

    measure_exe = control['comsuitedir'] + '/ComHartree'
    
    run_string=measure_exe
    cmd = run_string
    
    print(cmd, file=control['h_log'])
    
    with open('./comhartree.out', 'w') as logfile, open('./comhartree.err', 'w') as errfile :
        ret = subprocess.call(cmd,shell=True, stdout=logfile, stderr=errfile)
        if ret != 0:
            print("Error in ComHartree. Check evalsim.err for error message.", file=control['h_log'])
            sys.exit()
    print("measure hartree self-energy done", file=control['h_log'])
    if (control['method']=='lqsgw+dmft'):
        iter_string='_'+str(control['iter_num_impurity'])
    elif (control['method']=='lda+dmft'):
        iter_string='_'+str(control['iter_num_outer'])+'_'+str(control['iter_num_impurity'])    
    shutil.copy("./comhartree.out", "./comhartree"+iter_string+'.log')

    if (imp[key]['para']):                
        hartree_mat=reshape(loadtxt('./hartree.dat'), (nimp_orb, nimp_orb,5), order='F')[:,:,3]
        hartree_vec=imp_from_mat_to_array(hartree_mat,imp[key]['impurity_matrix'])            
    else:
        n_iio=amax(imp[key]['impurity_matrix'])
        hartree_mat=reshape(loadtxt('./hartree.dat'), (nimp_orb, nimp_orb,2,5), order='F')[:,:,:,3]
        hartree_vec=zeros(n_iio*2, dtype='complex')
        hartree_vec[:n_iio]=imp_from_mat_to_array(hartree_mat[:,:,0],imp[key]['impurity_matrix'])
        hartree_vec[n_iio:]=imp_from_mat_to_array(hartree_mat[:,:,1],imp[key]['impurity_matrix'])
    ...
    sigma_bare=read_function_from_jsonfile('./observables.json',"self-energy")

    for iomega in range(control['n_omega']):
        sigma_bare[iomega,:]=sigma_bare[iomega,:]-hartree_vec    
    ...
    return green, sigma_bare, sigma, sigma_to_delta, hartree_vec
...
def run_dc(control,imp):
    if ('dc_mat_to_read' in control):
        ...
        if (control['method']=='lda+dmft'):
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                nimp_orb=len(imp[str(abs(ii))]['impurity_matrix'])
                dc=reshape(loadtxt(control['dc_directory']+'/'+str(abs(ii))+'/sig_gw_mat.dat')[0,1:], (2,nimp_orb,nimp_orb), order='F')
                for jj in range(nimp_orb):
                    for kk in range(nimp_orb):
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                tempdat=reshape(loadtxt(control['dc_directory']+'/'+str(abs(ii))+'/sig_gw_mat.dat')[:,1:], (control['n_omega'],2,nimp_orb,nimp_orb), order='F')
            ...
            for jj in range(control['n_omega']):
                ...
                for ii in sorted(set(control['impurity_problem_equivalence'])):
                    # sig_dc_new=dot(dot(trans_basis[str(ii)], sig_dc[str(ii)][jj,:,:]), conj(transpose(trans_basis[str(ii)])))
            ...
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                tempdat=reshape(loadtxt(control['dc_directory']+'/'+str(abs(ii))+'/hartree.dat')[:,3:], (nimp_orb,nimp_orb,2), order='F')
                sig_dc_h[str(ii)]=tempdat[:,:,0]+tempdat[:,:,1]*1j

            sig_table=[]
            hf_header=control['sig_header'][1:]
            hf_header[0]='# '+hf_header[0]
            for ii in sorted(set(control['impurity_problem_equivalence'])):
                dc_vec=imp_from_mat_to_array(sig_dc_h[str(abs(ii))],imp[str(abs(ii))]['impurity_matrix'])
                sig_table.append(np.reshape(np.stack((dc_vec.real, dc_vec.imag), 0), (len(dc_vec)*2), order='F').tolist())

            with open(control['top_dir']+'/sig_hartree.dat', 'w') as outputfile:
                outputfile.write(tabulate(sig_table, headers=hf_header, floatfmt=".12f", numalign="right",  tablefmt="plain"))                

            shutil.copy(control['top_dir']+'/sig_hartree.dat', control['impurity_directory'])
