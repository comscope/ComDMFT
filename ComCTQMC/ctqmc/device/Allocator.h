...
namespace imp {
    ...
    public:
        ...
        struct Data {
            ...
            Data(PtrImpl ptrImpl) : ptrImpl_(ptrImpl), ptr_(reinterpret_cast<T*>(reinterpret_cast<std::uintptr_t>(ptrImpl_->ptr + (alignof(T) - 1)) & -alignof(T))) {};
        ...
        private:
            ...
            friend class AllocDevice;
        };
        
        AllocDevice() = delete;
        AllocDevice(std::size_t bytes) : bytes_(bytes), memory_(nullptr)  {
            cudaError_t check = cudaMalloc(reinterpret_cast<void**>(&memory_), bytes_);
            if(cudaSuccess != check) throw std::runtime_error(cudaGetErrorString(check));
        ...
        AllocDevice(AllocDevice const&) = delete;
        AllocDevice(AllocDevice&&) = delete;
        AllocDevice& operator=(AllocDevice const&) = delete;
        AllocDevice& operator=(AllocDevice&&) = delete;
        
        template<typename T, typename Enable = typename std::enable_if<alignof(T)%alignof(double) == 0>::type>
        Data<T> get(std::size_t size) {
            if(!size) throw std::runtime_error("Allocation of 0 bytes !"); // do we need this ???

            dummy_->bytes = sizeof(T)*size + alignof(T) - alignof(double); // we always allocate bytes = multiple of alignof(double) like this because of sfinae !! does is matter where memory ends ????
        ...
        template<typename T>
        Data<T> null_ptr() { return all_.begin();}  // needed for imp::Vector misery ... not sure if this is the best solution.
        ...
        ~AllocDevice() {
            if(free_.size() != 1) throw std::runtime_error("Memory leak");

            if((*free_.begin())->ptr != memory_ || (*free_.begin())->bytes != bytes_) throw std::runtime_error("Memory leak");
