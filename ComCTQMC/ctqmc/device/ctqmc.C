...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::size_t const streamsPerDevice = jParams("streams per device").int64();
        ...
        for(int id = 0; id < deviceCount; ++id) std::cout << " " << &pciIds[id*imp::pci_id_size()];  std::cout << std::endl;

        mpi::gather(pciIds, mpi::master); mpi::gather(nodeNames, mpi::master);
        
        std::vector<char> pciId; std::vector<std::size_t> mcIds;
        if(mpi::rank() == mpi::master) {
            pciId.resize(mpi::number_of_workers()*imp::pci_id_size(), '\0');
            
            std::map<std::string, std::set<std::string>> guard; std::size_t mcId = 0;
            for(int rank = 0; rank < mpi::number_of_workers(); ++rank) {
                ...
                for(unsigned int id = 0; id < deviceCount; ++id) {
                    ...
                    if(!guard[nodeName].count(currentPciId)) {
                        guard[nodeName].insert(currentPciId);
                ...
                mcId += pciId[rank*imp::pci_id_size()] != '\0' ? streamsPerDevice : 1;
                mcIds.push_back(mcId);
            }
        }
        ...
        for(std::size_t mcId = mcIds.front(); mcId < mcIds.back(); ++mcId) jSimulation(mcId - mcIds.front()) = jsx::int64_t(mcId);
        ...
        if(mode) {
            ...
            imp::init_device(pciId);
            mc::MonteCarlo<imp::AllocDevice>(jParams, jSimulation);
            imp::release_device();
        } else {
            std::cout << "Rank " << mpi::rank() << " gets simulations [" << mcIds.front() << ", " << mcIds.back() << ") and uses host" << std::endl;
            
            mc::MonteCarlo<imp::AllocHost>(jParams, jSimulation);
        }
