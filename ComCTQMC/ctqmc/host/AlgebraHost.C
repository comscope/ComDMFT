...
void imp::copyEvolveL(Matrix<AllocHost>& dest, Vector<AllocHost> const& prop, Matrix<AllocHost> const& source, itf::Batcher& batcher) {
...
void imp::mult(Matrix<AllocHost>& dest, Matrix<AllocHost> const& L, Matrix<AllocHost> const& R, itf::Batcher& batcher) {
...
void imp::evolveL(Vector<AllocHost> const& prop, Matrix<AllocHost>& arg, itf::Batcher& batcher) {
	arg.exponent() += prop.exponent(); int const inc = 1;
	for(int i = 0; i < arg.I(); ++i) dscal_(&arg.J(), prop.data().ptr() + i, arg.data().ptr() + i*arg.J(), &inc);
}

void imp::trace(ut::Zahl* Z, ut::Zahl* accZ, Matrix<AllocHost> const& matrix, itf::Batcher& batcher) {
...
void imp::traceAtB(ut::Zahl* Z, ut::Zahl* accZ, Matrix<AllocHost> const& At, Matrix<AllocHost> const& B, itf::Batcher& batcher) {
...
void imp::norm(double* norm, Matrix<AllocHost> const& matrix, itf::Batcher& batcher) {
	int inc = 1; int n = matrix.I()*matrix.J();
    *norm = std::log(dnrm2_(&n, matrix.data().ptr(), &inc)) + matrix.exponent();
}

void imp::density_matrix(Matrix<AllocHost>& dest, Matrix<AllocHost> const& Bt, Vector<AllocHost> const& prop, Matrix<AllocHost> const& A, itf::Batcher& batcher) {
    dest.I() = A.I(); dest.J() = Bt.I(); dest.exponent() = A.exponent() + Bt.exponent();
    char transNo = 'n'; char transYes = 't'; double one = 1.; double zero = .0;
    dgemm_(&transYes, &transNo, &Bt.I(), &A.I(), &A.J(), &one, Bt.data().ptr(), &Bt.J(), A.data().ptr(), &A.J(), &zero, dest.data().ptr(), &dest.J());
    
    dest.exponent() += prop.exponent(); double const deltaTime = -prop.time(); // this is confusing, change time -> -time
    auto d = dest.data().ptr(); auto const p = prop.data().ptr(); auto const e = prop.energies().data().ptr();
...
void imp::axpy(Matrix<AllocHost>& dest, ut::Zahl const& fact, Matrix<AllocHost> const& source, itf::Batcher& batcher) {
...
/*
void imp::mult(double beta, Matrix<AllocHost>& dest, double alpha, Matrix<AllocHost> const& L, Matrix<AllocHost> const& R) {
    dest.I() = L.I(); dest.J() = R.J(); dest.exponent() = L.exponent() + R.exponent(); char transNo = 'n'; 
    dgemm_(&transNo, &transNo, &R.J(), &L.I(), &L.J(), &alpha, R.data().ptr(), &R.J(), L.data().ptr(), &L.J(), &beta, dest.data().ptr(), &dest.J());
}

void imp::diag_comm(Energies<AllocHost> const& eig, Matrix<AllocHost>& arg) {
    for(int i = 0; i < arg.I(); ++i)
        for(int j = 0; j < arg.J(); ++j)
            arg.data().ptr()[j + arg.J()*i] *= (eig.data().ptr()[i] - eig.data().ptr()[j]);
}
*/
